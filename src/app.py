...
class App(ctk.CTk):
    ...
    def create_layout(self):
        ...
        self.label_heading.configure(
            text="TRANSMITTER - FILE MOVER", font=("Inclusive Sans", 25)
        )
    ...
    def update_label(self, value):
        ...
        hours, minutes, seconds = AudioPlayer.get_duration_as_hms(total_seconds)
    ...
    def manual_select(self):
        ...
        if path:
            ...
            self.current_files: list = self._class_based_files.load_files(
                path
            )  # list of files for moving etc
            self.A20_path = path
            ...
            if not self.current_files:
                ...
                for file_instance in self.current_files:
                    file_dict: dict = self._wav_info_get.info_getter(
    ...
    def add_to_details(self, index):
        index: tuple = (
            self.a20_listbox.curselection()
        )  # gets the selected item index in the listbox
            ...
            if not self.current_files:
                ...
                for file_instance in self.current_files:
                    ...
                    self.options_label.configure(
                        text=f"Transmitter: {file_label_item}",
    ...
    def show_today(self):
        today_date = self._controller.global_time
        print(today_date)
    ...
    def call_move_files(self) -> None:
        if self.A20_path and self.folder_path:
            ...
            file_match = self._controller.match_files_to_folder(
                folder_path=self.folder_path, tx_path=self.A20_path
            )
