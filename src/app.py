...
# Folder Stuff
        ...
        if a20_mount_point:
            ...
            try:
                ...
                for file_info in received_file_list:
                    ...
                    display_text: str = f"{file_info['count']}-{file_info['file_name']} : {file_info['mb']} MB : length-{file_info['length']} : start-{file_info['start_tc']}\n\n"
    ...
    def update_progress(self, copied_size, file_size, file, textbox, total_files, file_count):
        percentage = (copied_size / file_size) * 100
        overall_percentage = ((file_count / total_files) * 100) + ((copied_size / file_size) * (100 / total_files))

        # Update the terminal-based progress bar
        print(f"Copying {file}: {percentage:.2f}% complete")

        # Update the text-based progress bar in the GUI
        progress_text = self.create_text_progress_bar(overall_percentage)
        if textbox:
            self.terminal_textbox.delete("1.0", ctk.END)  # Clear previous content
            self.terminal_textbox.insert(ctk.END, progress_text)  # Insert new progress bar
            self.terminal_textbox.see(ctk.END)  # Scroll to the end (optional)
