...
class WavInfoGet:
    ...
    def info_getter(self, passed_file) -> dict:
        if passed_file:
            ...
            file_run_time_float = samples / sample_rate if samples and sample_rate else None
            file_run_time_int = round(file_run_time_float) if file_run_time_float else None
            time_delta = datetime.timedelta(seconds=file_run_time_int) if file_run_time_int else None

            # Extract frame rate from description
            match = re.search(r"sSPEED=([\d.]+-ND)", description) if description else None
