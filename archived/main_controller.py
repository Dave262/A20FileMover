...
class MainController:
    ...
    def select_folder_path(self):
        folder_path: str = filedialog.askdirectory(
            ...
            title="please select your bodypack folder path",
        )
    ...
    def select_A20_path(
        ...
        A20_path: str = filedialog.askdirectory(
    ...
    def move_files(self, A20_path, folder_path, textbox=None, progress_callback=None):
        if A20_path and folder_path:
            print(
                f"def - move_files in main controller can see {A20_path} : {folder_path}"
            )
            ...
            for file in files:
                ...
                if name_only:
                    ...
                    for folder in os.listdir(folder_path):
                        if name_only == folder:
                            print(
                                f"there's a match! : {file} corresponds to the folder {folder}"
                            )
                            match_found = True
                    ...
                    try:
                        ...
                        with open(src_path, "rb") as src_file, open(
                            dst_path, "wb"
                        ) as dst_file:
                            with tqdm(
                                total=file_size,
                                ...
                                for chunk in iter(
                                    lambda: src_file.read(1024 * 1024), b""
                                ):
    ...
    def give_list_of_attributes_for_tx_files(self):
        ...
        for index, file in enumerate(os.listdir(a20_mount_point), start=1):
            ...
            try:
                ...
                if fmt_info:
                    print(
                        f"{index}_{file} - bit depth: {fmt_info.bits_per_sample} - sample rate: {fmt_info.sample_rate}"
                    )
            except wavinfo.riff_parser.WavInfoEOFError:
                print(
                    f"EOF error encountered while reading {file_path}. The file may be corrupted or incomplete."
                )
            except Exception as e:
